...
> ⚠️ **Windows 필독 — 설치 직후 `supabase`/`node` 가 "인식되지 않습니다"로 나오면:**
> 방금 설치한 프로그램은 **이미 열려 있던 터미널·VS Code 창에는 즉시 반영되지 않습니다**(PATH 갱신 문제).
> 같은 창에서 계속 시도하지 말고, 둘 중 하나로 해결하세요.
> 1. (권장) **VS Code 를 완전히 종료했다가 다시 켜기** → 그러면 새 터미널에서 명령이 잡힙니다.
>    Claude Code 로 진행 중이었다면 재시작 후 `claude` 를 다시 실행하고 **"이어서 설치해줘"** 라고 하면 됩니다.
> 2. (빠른 우회) **새 cmd 창을 따로 열어** 거기서 그 명령(예: `supabase login`)을 실행하세요. 새 창은 바로 됩니다.
